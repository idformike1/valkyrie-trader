...
def demo_replay(strategy, csv_file='backtest_output.csv'):
    ...
    # default volume (not used in chart anymore)
...
# -------------------------------
# Flask App with Final Frontend (no volume, proper layout)
# -------------------------------
app = Flask(__name__)
CORS(app)
...
<html>
<head>
    <title>Valkyrie Trader - Final</title>
    <style>
...
<script>
    ...
    function computeHeikinAshi(candles) {
        ...
        for (let i = 0; i < candles.length; i++) {
            ...
            result.push({ x: c.x, open: haOpen, high: haHigh, low: haLow, close: haClose });
            prevHAOpen = haOpen;
            prevHAClose = haClose;
        }
        return result;
    ...
    // Resample candles (timezone-safe)
    function resampleCandles(candles, minutes) {
        if (minutes <= 1) return candles;
        const groups = new Map();
        for (let c of candles) {
            let d = new Date(c.x);
            let bucket = Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate(), d.getUTCHours(), Math.floor(d.getUTCMinutes() / minutes) * minutes, 0, 0);
    ...
    function startDemo() {
        if (activeDemo) {
            addLog('Demo already running. Stop first.');
            return;
        }
    ...
    document.getElementById('haToggle').onchange = (e) => {
        ...
        updateChart();
        addLog(`Heikin-Ashi ${useHeikinAshi ? 'ON' : 'OFF'}`);
    };

    window.onload = () => {
        fetch('/status').then(res => res.json()).then(updateStatus);
        addLog('Final dashboard ready. Click DEMO to start.');
    };
</script>
</body>
</html>
