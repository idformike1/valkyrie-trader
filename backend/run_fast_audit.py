...
def run_fast_audit():
    ...
    for idx in target_trade_indices:
        ...
        match = (
            ...
            abs(t_leg.gross_pnl - t_dyn.gross_pnl) < 1e-4 and
        ...
        "signal": {
            ...
            "entry_condition": {
                ...
                "conditions": [
                    ...
                    { "type": "greater_than", "params": { "primary": "vol_ratio[-1]", "value": -1.0 } }
                ]
            }
        },
        "contract": {
    ...
    def get_example_trade_str(exit_list):
        if not exit_list:
            return "N/A"
        trade = exit_list[0]
        if trade in gar_summary.trades:
            return f"GAR Trade #{gar_summary.trades.index(trade)+1} (Net PnL: {trade.net_pnl:.2f})"
        elif trade in complex_summary.trades:
            return f"Complex Trade #{complex_summary.trades.index(trade)+1} (Net PnL: {trade.net_pnl:.2f})"
        elif trade in legacy_summary.trades:
            return f"Legacy Trade #{legacy_summary.trades.index(trade)+1} (Net PnL: {trade.net_pnl:.2f})"
        else:
            return f"Trade (Net PnL: {trade.net_pnl:.2f})"
...
|---|---|---|
| Stop Loss | {len(sl_exits)} | {get_example_trade_str(sl_exits)} |
| Take Profit | {len(tp_exits)} | {get_example_trade_str(tp_exits)} |
| Trailing SL | {len(trailing_exits)} | {get_example_trade_str(trailing_exits)} |
